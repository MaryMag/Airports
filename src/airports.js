...
class Airport extends Component {
  ...
  render() {
    ...
    var toDescription = this.state.airports[to]
    var from = this.state.from;
    var fromDescription = this.state.airports[from]

    var toString  = this.formatDescription(toDescription);
    var fromString = this.formatDescription(fromDescription);

    return (
      <form className="airports">
        <img className="airports-img" src="https://image.flaticon.com/icons/png/512/476/476505.png" /> 
        <h1>Grab your ticket</h1>
        <input type="text" name="from" onChange={this.handleFromChange.bind(this)} defaultValue={from}/>
        <div className="about from">{fromString}</div>
        <input type="text" name="to" onChange={this.handleToChange.bind(this)} defaultValue={to}/>
